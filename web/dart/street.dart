...
class Street
{
	...
	Street(String streetName)
	{
		...
		if(chat.username != null) sendLeftMessage(label);
		...
		if(quoins != null) quoins.clear();

		if(otherPlayers != null) otherPlayers.clear();
	}
	...
	Future<List> load()
	{
		...
		if(_data['music'] != null) setSong(_data['music']);
		...
		decos.load(setLoadingPercent).then((_)
		{
			...
			interactionCanvas.classes.add('streetcanvas');
			interactionCanvas.style.pointerEvents = "auto";
			interactionCanvas.id = "interractions";
			interactionCanvas.style.width = bounds.width.toString() + "px";
			interactionCanvas.style.height = bounds.height.toString() + "px";
			interactionCanvas.style.position = 'absolute';
			interactionCanvas.attributes['ground_y'] = groundY.toString();
			...
			{
				DivElement decoCanvas = new DivElement()..classes.add('streetcanvas');
	...
	void applyFilters(Map layer, DivElement decoCanvas)
	{
		...
		new Map.from(layer['filters']).forEach((String filterName, int value)
		{
			...
			{
				if(value < 0) filters.add('brightness(' + (1 - (value / -100)).toString() + ')');
			...
			{
				if(value < 0) filters.add('contrast(' + (1 - (value / -100)).toString() + ')');
			...
			{
				if(value < 0) filters.add('saturation(' + (1 - (value / -100)).toString() + ')');
