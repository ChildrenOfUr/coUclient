...
{    
	...
	Street(String streetName)
	{
		...
		app.playerHolder.children.clear();
		app.playerHolder.style.width = bounds.width.toString()+'px';
		app.playerHolder.style.height = bounds.height.toString()+'px';
		app.playerHolder.classes.add('streetcanvas');
		app.playerHolder.style.position = "absolute";
		app.playerHolder.attributes["ground_y"] = "0";
		app.playerHolder.style.transform = "translateZ(0)";
	...
	{		
		...
		decos.load(setStreetLoadBar).then((_)
        {
			...
			app.layers.append(gradientCanvas);
			...
			for(Map layer in new Map.from(_data['dynamic']['layers']).values)
			{
				...
				app.layers.append(decoCanvas);
			}
			
			app.layers.append(interactionCanvas);
	...
	render()
	{
		...
		if(camera.dirty)
		{
			num currentPercentX = camera.getX() / (bounds.width - app.worldElement.clientWidth);
			num currentPercentY = camera.getY() / (bounds.height - app.worldElement.clientHeight);
			
			//modify left and top for parallaxing
			Map<String,DivElement> transforms = new Map();
			for(DivElement canvas in app.worldElement.querySelectorAll('.streetcanvas'))
			{
				int canvasWidth = int.parse(canvas.style.width.replaceAll('px', ''));
				int canvasHeight = int.parse(canvas.style.height.replaceAll('px', ''));
				double offsetX = (canvasWidth - app.worldElement.clientWidth) * currentPercentX;
				double offsetY = (canvasHeight - app.worldElement.clientHeight) * currentPercentY;
...
Future load_streets()
{
	app.loadStatus.text = "Loading Streets";
	...
	new Asset('./assets/streets.json').load().then((Asset streetList) 
	{
		...
		for (String url in streetList.get().values)
			toLoad.add(new Asset(url).load(app.loadStatus2));
...
Element mapLoadingScreen = querySelector('#streetLoading');
Element mapLoadingBar = querySelector('#newLocation .bar .percent');
