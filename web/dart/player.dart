...
class Player
{
	...
	Player([String name])
	{
		...
		app.worldElement.append(playerParentElement);
	}
	...
	{
		num translateX = posX, translateY = app.worldElement.clientHeight - height;
		num camX = camera.getX(), camY = camera.getY();
		if(posX > currentStreet.bounds.width - width/2 - app.worldElement.clientWidth/2)
		{
			camX = currentStreet.bounds.width - app.worldElement.clientWidth;
			translateX = posX - currentStreet.bounds.width + app.worldElement.clientWidth; //allow character to move to screen right
		}
		else if(posX + width/2 > app.worldElement.clientWidth/2)
		{
			camX = posX + width/2 - app.worldElement.clientWidth/2;
			translateX = app.worldElement.clientWidth/2 - width/2; //keep character in center of screen
		}
		else
			camX = 0;
		
		if(posY + height/2 < app.worldElement.clientHeight/2)
		{
			camY = 0;
			translateY = posY;
		}
		else if(posY < currentStreet.bounds.height - height/2 - app.worldElement.clientHeight/2)
		{
			num yDistanceFromBottom = currentStreet.bounds.height - posY - height/2;
			camY = currentStreet.bounds.height - (yDistanceFromBottom + app.worldElement.clientHeight/2);
			translateY = app.worldElement.clientHeight/2 - height/2;
		}
		else
		{
			camY = currentStreet.bounds.height - app.worldElement.clientHeight;
			translateY = app.worldElement.clientHeight - (currentStreet.bounds.height - posY);
		}
	...
	void checkCollision(Rectangle avatarRect, Element element)
	{
		...
		{
			if(ASSET['drop'] != null && app.volume > 0)
			{
