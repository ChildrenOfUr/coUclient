...
class Metabolics
{
	...
	void init()
	{
		//load currants (for now)
  		if(localStorage["currants"] != null)
    		setCurrants(int.parse(localStorage["currants"]));
	}
	...
	setMood(int newValue)
	{
		...
		_mood = newValue;
		ui.currMoodText.parent.classes.toggle('changed', true);
		Timer t = new Timer(new Duration(seconds:1),() => ui.currMoodText.parent.classes.toggle('changed'));
		ui.currMoodText.text=_mood.toString();
		ui.moodPercent.text=((100*((_mood/_maxmood))).toInt()).toString();
		ui.moodmeterImageLow.style.opacity = ((0.7-(_mood/_maxmood))).toString();
		
		if (_mood <= 0)
			ui.moodmeterImageEmpty.style.opacity = 1.toString();
		else
			ui.moodmeterImageEmpty.style.opacity = 0.toString();
	}
