...
String multiplayerServer =
    "ws://${Configs.websocketServerAddress}/playerUpdate";
String streetEventServer =
    "ws://${Configs.websocketServerAddress}/streetUpdate";
String joined = "",
    creatingPlayer = "";
...
void sendJoinedMessage(String streetName, [String tsid]) {
  if (joined != streetName &&
      streetSocket != null &&
      streetSocket.readyState == WebSocket.OPEN) {
    Map map = new Map();
...
_setupStreetSocket(String streetName) {
  ...
  streetSocket.onMessage.listen((MessageEvent event) {
    ...
    if (map['label'] != null && currentStreet.label != map['label']) return;
    ...
    if (map['giveItem'] != null) {
      for (int i = 0; i < map['num']; i++) addItemToInventory(map);
      return;
    }
    ...
    if (map['openWindow'] != null) {
      if (map['openWindow'] == 'vendorSell') new VendorWindow().call(map,
          sellMode: true);
      ...
      if (element == null) addNPC(npcMap);
        ...
        if (npc != null) {
          if (npc.animation.url != npcMap["url"]) //new animation
            ...
            List<int> frameList = [];
            for (int i = 0; i < npcMap['numFrames']; i++) frameList.add(i);
