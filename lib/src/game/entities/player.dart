...
class Player {
  ...
  Player(this.username) {
    ...
    if (posX == 1.0 && posY == 0.0) {
      ...
      for (Platform platform in currentStreet.platforms) {
        if (leftmost == null || platform.start.x < leftmost.start.x) leftmost =
            platform;
  ...
  remove() {
    for (Element element in this.playerParentElement.children) element.remove();
  }
  ...
  Future<List<Animation>> loadAnimations() {
    ...
    List<int> idleFrames = [],
        ...
        landFrames,
        climbFrames = [];
    for (int i = 0; i < 57; i++) idleFrames.add(i);
    for (int i = 0; i < 12; i++) baseFrames.add(i);
    for (int i = 0; i < 16; i++) jumpUpFrames.add(i);
    for (int i = 0; i < 19; i++) climbFrames.add(i);
    fallDownFrames = [16, 17, 18, 19, 20, 21, 22, 23];
    landFrames = [24, 25, 26, 27, 28, 29, 30, 31, 32];

    List<Future> futures = new List();

    futures.add(HttpRequest
        .requestCrossOrigin(
      ...
      animations['idle'] = new Animation(idle, "idle", 2, 29, idleFrames,
  ...
  update(double dt) {
    ...
    if (chatBubble != null) chatBubble.update(dt);

    if (doPhysicsApply && inputManager.upKey == true) {
      bool found = false;
      Rectangle playerRect = new Rectangle(posX,
          posY + currentStreet.streetData['dynamic']['ground_y'], width,
          height - 15);
      for (Ladder ladder in currentStreet.ladders) {
        if (intersect(ladder.bounds, playerRect)) {
          ...
          if (playerRect.top + playerRect.height < ladder.bounds.top) break;
      ...
      for (Ladder ladder in currentStreet.ladders) {
        if (intersect(ladder.bounds, playerRect)) {
          ...
          if (playerRect.top + playerRect.height >
              ladder.bounds.top + ladder.bounds.height) break;
    ...
    if (doPhysicsApply &&
        inputManager.downKey == false &&
        inputManager.upKey == false) {
      bool found = false;
      Rectangle playerRect = new Rectangle(posX,
          posY + currentStreet.streetData['dynamic']['ground_y'], width,
          height);
    ...
    bool jump() {
      if (!jumping) {
        ...
        if (jumpWatch.elapsedMilliseconds == 0) {
          jumpWatch.start();
        }
        
        return true; // jumped
      } else {
        return false; // was already jumping
      }
    }
    ...
    if (inputManager.jumpKey == true) {
      switch (jumps) {
        ...
        case 1:
          ...
          break;
        case 2:
          if (jumpWatch.elapsedMilliseconds.toInt() < 1500) {
            if (tripleJump() == false) {
              jumping = false;
            }
          } else {
    ...
    if (doPhysicsApply && !climbingUp && !climbingDown) {
      ...
      if (inputManager.downKey == true) posY += speed * dt;
      if (inputManager.upKey == true) posY -= speed * dt;
    }

    if (posX < 0) posX = 0.0;
    if (posX > currentStreet.bounds.width - width) posX =
        currentStreet.bounds.width - width;
    ...
    if (doPhysicsApply && !climbingDown && !climbingUp && yVel >= 0) {
      ...
      if (bestPlatform != null) {
        num goingTo =
            posY + height + currentStreet.streetData['dynamic']['ground_y'];
        num slope = (bestPlatform.end.y - bestPlatform.start.y) /
            (bestPlatform.end.x - bestPlatform.start.x);
        num yInt = bestPlatform.start.y - slope * bestPlatform.start.x;
        num lineY = slope * x + yInt;

        if (goingTo >= lineY) {
          posY =
              lineY - height - currentStreet.streetData['dynamic']['ground_y'];
          yVel = 0;
          jumping = false;
        }
      }
    ...
    querySelectorAll(".quoin")
        .forEach((Element element) => checkCollision(element));

    intersectingObjects = {};
    querySelectorAll(".entity").forEach((Element element) {
      num left =
          num.parse(element.attributes['translatex'].replaceAll("px", ""));
      num top =
          num.parse(element.attributes['translatey'].replaceAll("px", ""));
      ...
      if (intersect(avatarRect, entityRect)) {
        if (entities[element.id] != null) entities[element.id].updateGlow(true);

        intersectingObjects[element.id] = entityRect;
      } else {
        if (entities[element.id] != null) entities[element.id]
            .updateGlow(false);
      }
    });
  }
  ...
  void render() {
    if (currentAnimation != null &&
        ...
        currentAnimation.dirty) {
      if (!firstRender) {
        Rectangle avatarRect = new Rectangle(
            posX, posY, currentAnimation.width, currentAnimation.height);
        if (!intersect(camera.visibleRect, avatarRect)) return;
      }

      firstRender = false;

      if (playerCanvas.width != currentAnimation.width ||
      ...
      } else playerCanvas.context2D.clearRect(
          0, 0, currentAnimation.width, currentAnimation.height);

      Rectangle destRect =
          new Rectangle(0, 0, currentAnimation.width, currentAnimation.height);
      playerCanvas.context2D.drawImageToRect(
          currentAnimation.spritesheet, destRect,
          sourceRect: currentAnimation.sourceRect);
  ...
  void updateAnimation(double dt) {
    ...
    if (!moving && !jumping && !climbing) currentAnimation = animations['idle'];
    else {
      ...
      if (climbing) {
        ...
        if (moving && !jumping) currentAnimation = animations['base'];
  ...
  void updateTransform() {
    ...
    num prevX,
        prevY,
        prevCamX = camera.getX(),
        prevCamY = camera.getY();
    if (xattr != null) prevX = num.parse(xattr);
    else prevX = 0;
    if (yattr != null) prevY = num.parse(yattr);
    else prevY = 0;
    ...
    num camX = camera.getX(),
        camY = camera.getY();
    if (posX >
        currentStreet.bounds.width - width / 2 - view.worldElementWidth / 2) {
      camX = currentStreet.bounds.width - view.worldElementWidth;
      translateX = posX -
          currentStreet.bounds.width +
          view.worldElementWidth; //allow character to move to screen right
      ...
      translateX = view.worldElementWidth / 2 -
          width / 2; //keep character in center of screen
    } else camX = 0;
    ...
    if (!facingRight) {
      ...
      playerName.style.transform =
          'translateY(-100%) translateY(-34px) scale3d(-1,1,1)';

      if (chatBubble != null) chatBubble.textElement.style.transform =
          'scale3d(-1,1,1)';
    } else {
      playerName.style.transform =
          'translateY(-100%) translateY(-34px) scale3d(1,1,1)';
    ...
    num diffX = translateX - prevX,
        diffY = translateY - prevY;
  }
  ...
  void checkCollision(Element element) {
    ...
    if (element.attributes['collected'] == "true" ||
        element.attributes['checking'] == "true") return;

    CanvasElement canvas = element as CanvasElement;
    num left = num.parse(canvas.style.left.replaceAll("px", ""));
    num top = currentStreet.bounds.height -
        num.parse(canvas.style.bottom.replaceAll("px", "")) -
        canvas.height;
    ...
    if (intersect(avatarRect, quoinRect)) {
      ...
      new Timer(new Duration(seconds: 2),
          () => _removeCircleExpand(querySelector("#qq" + element.id)));
      new Timer(new Duration(milliseconds: 800),
          () => _removeCircleExpand(querySelector("#q" + element.id)));
  ...
  void _removeCircleExpand(Element element) {
    if (element != null) element.classes.remove("circleExpand");
  }
  ...
  Platform _getBestPlatform(num cameFrom) {
    ...
    num x = posX + width / 2;
    num feetY =
        cameFrom + height + currentStreet.streetData['dynamic']['ground_y'];
    num bestDiffY = 1000;

    for (Platform platform in currentStreet.platforms) {
      if (x >= platform.start.x && x <= platform.end.x) {
        num slope = (platform.end.y - platform.start.y) /
            (platform.end.x - platform.start.x);
        num yInt = platform.start.y - slope * platform.start.x;
        num lineY = slope * x + yInt;
        num diffY = (feetY - lineY).abs();

        if (bestPlatform == null) bestPlatform = platform;
        else {
          if ((lineY >= feetY ||
                  !jumping && feetY > lineY && feetY - (height / 2) < lineY) &&
              diffY < bestDiffY) {
