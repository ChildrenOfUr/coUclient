...
class DetailsWindow
{
	...
	static Element create(Map item, Map vendorMap, {bool sellMode : false})
	{
    	...
    	{
    		if(!sellMode && metabolics.getCurrants() < item['price']*numToBuy)
    			return;
    		
    		int newValue = metabolics.getCurrants()-item['price']*numToBuy;
    		if(sellMode)
    			newValue = metabolics.getCurrants()+(item['price']*.7*numToBuy)~/1;
    		metabolics.setCurrants(newValue);
    		querySelector("#NumCurrants").text = " ${ui.commaFormatter.format(metabolics.getCurrants())} currants";
