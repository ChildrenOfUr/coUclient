...
class Chat {
  ...
  Chat(this.title) {
    ...
    if (title != "Local Chat") {
      ...
      if (localChat != null) view.panel.insertBefore(
          conversationElement, localChat.conversationElement);
      else view.panel.append(conversationElement);
  ...
  void processInput(TextInputElement input) {
    input.onKeyDown.listen(
        (KeyboardEvent key) //onKeyUp seems to be too late to prevent TAB's default behavior
        {
      if (key.keyCode == 38 &&
              inputHistoryPointer < inputHistory.length) //pressed up arrow
          {
        input.value = inputHistory.elementAt(inputHistoryPointer);
        if (inputHistoryPointer <
            inputHistory.length - 1) inputHistoryPointer++;
      }
      ...
      if (input.value != "" &&
              key.keyCode ==
                  9) //tab key, try to complete a user's name or an emoticon
          {
        key.preventDefault();

        if (input.value
                .endsWith(":")) //look for an emoticon instead of a username
            {
          ...
          if (value.length > 1 &&
                      value
                          ...
                          .startsWith(" :") ||
                  value.length == 1 &&
                      value.startsWith(":")) //start of new emoticon
              {
            input.value = value.substring(0, value.lastIndexOf(":") + 1) +
                EMOTICONS.elementAt(emoticonPointer) +
                ":";
            emoticonPointer++;
            if (emoticonPointer == EMOTICONS.length) emoticonPointer = 0;
                  ...
                  !value
                      .substring(value.lastIndexOf(":") - 1)
                      .startsWith(" :")) //change existing emoticon choice
              {
            ...
            while (count < EMOTICONS.length * 2) {
              ...
              if (setNext) {
                input.value = value.substring(0, lastColon -
                        EMOTICONS.elementAt(emoticonPointer).length) +
                    name +
                    ":";
                emoticonPointer++;
                if (emoticonPointer == EMOTICONS.length) emoticonPointer = 0;
  ...
  tabComplete(TextInputElement input, KeyboardEvent k) async {
    ...
    if (title != channel) channel = currentStreet.label;
    String url =
